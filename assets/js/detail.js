...
sidebar();
const randomPrice = Math.floor(Math.random() * (300 - 250)) + 250;
...
fetchDataFromServer(
  ...
  function (movie) {
    ...
    movieDetail.innerHTML = `
      ...
      "w1280" || "original"
                ...
                <div class="title-wrapper">
                    ...
                    <h3 class="title-large"> ₹ ${randomPrice}</h3>
                    <button id="book-ticket-btn" type="button"> Book Tickets </button>
                </div>
    ...
    `;
    const bookTicketButton = movieDetail.querySelector("#book-ticket-btn");

    bookTicketButton.addEventListener("click", function () {
  // Store movie title and price in localStorage
  localStorage.setItem("selectedMovieTitle", title);
  localStorage.setItem("selectedMoviePrice", randomPrice);
